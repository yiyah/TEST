...
echo scan time about 2 minutes
echo.

set networkSegment=192.168.10.0
...
for /l %%i in (1,1,100) do ping -w 10 -n 1 %i1%.%i2%.%i3%.%%i | find "TTL="

set time_end=%time%

:: get totalTime_minute
if %time_end:~3,2% lss %time_begin:~3,2% (set /a totalTime_minute = %time_begin:~3,2% + 60 - %time_end:~3,2%) else set /a totalTime_minute = %time_end:~3,2% - %time_begin:~3,2% 

:: get totalTime_second
if %time_end:~-5,2% lss %time_begin:~-5,2% (set /a totalTime_second = %time_begin:~-5,2% + 60 - %time_end:~-5,2% & set /a totalTime_minute = %totalTime% - 1) else set /a totalTime_second = %time_end:~-5,2% - %time_begin:~-5,2%
